...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::post('api/productions', function (Request $request) {
        return app(ProductionManagement::class)->store($request);
    })->name('api.productions.store');
    Route::put('/admin/materials/update-quantities/{id}', function (Request $request, $id) {
        return app(MaterialsController::class)->updateQuantities($request, $id);
    })->name('admin.materials.updateQuantities');
});
